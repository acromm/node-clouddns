...
CloudDNS.prototype.setAuth = function (callback) {
  ...
  request(authOptions, function (err, res, body) {
   ...
   if (Object.keys(common.failCodes).indexOf(statusCode) !== -1) {
    ...
    if (self.config.cdnUrl) {
      self.config.accountId = self.config.cdnUrl.split('/').pop().split('_').pop();
...
//
// ### function dnsUrl ()
// Return the root of the url to call
//
CloudDNS.prototype.dnsUrl = function dnsUrl() {
  var args = Array.prototype.slice.call(arguments);
  var qsArg = args[args.length-1]
  
  if (typeof qsArg === 'string' && qsArg.substring(0,1) == '?')
    args = args.slice(0, args.length-1);
  
  var queryParams = (typeof qsArg === 'string' && qsArg.substring(0,1) == '?') ? qsArg : '';
  var url = [this.config.dnsUrl].concat(args).join('/')  + '.json'+queryParams;
  //console.log('URL', url);
  return url;
};
...
// #### @callback {Function}
// Internal use to wrap raw domain objects in our Domain objects
//
...
// #### @callback {Function}
// Internal use to wrap raw record objects in our Record objects
//
...
CloudDNS.prototype.createDomain = function createDomain(domainObj) {
  ...
  this.rackspace(reqOpts, callback, function (body) {
	...
	self._wrapDomains(request.domains, callback);
  });
