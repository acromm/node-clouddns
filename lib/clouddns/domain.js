...
Domain.prototype = {
    ...
    //
    // ### function searchRecords (options, callback)
    // #### @options {Object}
    // #### @callback {Function}
    // Search for records of the domain. you can pass {options} with 'type' and 'name'
    //
    searchRecords: function getDetails(options, callback) {
        ...
        var reqOpts = {
            method: 'GET',
            uri: this._client.dnsUrl('domains', this.id, 'records', '?type='+options.type+'&name='+options.name),
            client: this._client
        }
...
*/
                self._client._wrapRecords(self, parsed.records, function(err, records) {
    ...
    removeRecords: function removeRecords(recordObj, callback) {
        ...
        //
        ,
    removeRecord: function removeRecord(recordId, callback) {
            var args = Array.prototype.slice.call(arguments),
                callback = args[args.length - 1]
            self = this;
            ...
            if (!recordId) {
                return callback(new Error("record Id is mandatory"));
            }

            var reqOpts = {
                method: 'DELETE',
                uri: this._client.dnsUrl('domains', this.id, 'records', recordId),
                client: this._client
            }
